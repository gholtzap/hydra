...
type UiState = {
  ...
  worktreeManagerFilter: WorktreeManagerFilter;
  repoBranchCache: Map<string, RepoBranchCacheEntry>;
  repoBranchLoading: Set<string>;
...
const ui: UiState = {
  ...
  worktreeManagerFilter: "all" as WorktreeManagerFilter,
  repoBranchCache: new Map<string, RepoBranchCacheEntry>(),
  repoBranchLoading: new Set<string>(),
...
const worktreeCloseDialog = document.getElementById("worktree-close-dialog") as HTMLDialogElement;
...
function filteredWorktreeManagerEntries(entries: ParallelWorktreeManagerEntry[]): ParallelWorktreeManagerEntry[] {
  if (ui.worktreeManagerFilter === "active") {
    return entries.filter((entry) => entry.kind === "active");
  }
  if (ui.worktreeManagerFilter === "inactive") {
    return entries.filter((entry) => entry.kind !== "active");
  }
  return entries;
}

function renderWorktreeManagerFilterControls() {
  const filters: Array<{ value: WorktreeManagerFilter; label: string }> = [
    { value: "all", label: "All" },
  ...
  return `
    <div class="worktree-filter-controls" role="group" aria-label="Worktree filter">
      ${filters.map((filter) => `
        <button
          type="button"
          class="worktree-filter-button ${ui.worktreeManagerFilter === filter.value ? "active" : ""}"
          data-action="worktree-filter"
          data-filter-value="${escapeAttribute(filter.value)}"
...
function renderRepoParallelWorktreeProjectPanel(repo: RepoSnapshot) {
  ...
  const allEntries = repoParallelWorktreeManagerEntries(repo);
  const entries = filteredWorktreeManagerEntries(allEntries);
  const activeEntries = allEntries.filter((entry) => entry.kind === "active");
    ...
    <section class="repo-worktree-panel">
      <div class="repo-worktree-header">
        ...
        <div class="repo-worktree-actions">
          ${renderWorktreeManagerFilterControls()}
              ...
              ${entries.map((entry) => `
                <div class="repo-worktree-row">
                  ...
                  <div class="settings-worktree-actions">
                    ${entry.kind === "active" ? `<button type="button" data-action="select-session" data-session-id="${escapeAttribute(entry.sessionId)}">Open</button>` : `<button type="button" data-action="resume-worktree" data-repo-id="${escapeAttribute(repo.id)}" data-worktree-path="${escapeAttribute(entry.path)}">Resume</button>`}
                    <button type="button" data-action="rename-worktree" data-repo-id="${escapeAttribute(repo.id)}" data-worktree-path="${escapeAttribute(entry.path)}" data-worktree-title="${escapeAttribute(entry.title)}">Rename</button>
                    <button type="button" data-action="delete-worktree" data-repo-id="${escapeAttribute(repo.id)}" data-worktree-path="${escapeAttribute(entry.path)}" data-session-id="${escapeAttribute(entry.sessionId)}" data-worktree-kind="${escapeAttribute(entry.kind)}">Delete</button>
...
async function renderSettingsDialog() {
  ...
  if (ui.settingsTab === "general" && repoId) {
    await ensureRepoBranches(repoId);
  }
      ...
      <div class="settings-group-body">
        ...
        <label class="settings-inline-field">
          ...
          <select
            ...
            data-repo-id="${escapeAttribute(repo.id)}"
          >
            ${renderBranchSelectOptions(repo, effectiveSettings.baseBranch || "main", [effectiveSettings.landingBranch])}
          </select>
        </label>
        <label class="settings-inline-field">
          <span class="row-title">Push to Branch</span>
          <select
            id="pref-repo-worktree-landing"
            data-repo-id="${escapeAttribute(repo.id)}"
          >
            ${renderBranchSelectOptions(repo, effectiveSettings.landingBranch || "main", [effectiveSettings.baseBranch])}
          </select>
        </label>
      </div>
    </section>
  ...
  return `
    <section class="settings-group-card settings-toggle-card">
      ...
      <div class="settings-group-body">
        ${
          ...
          <div class="settings-worktree-row">
            ...
            <div class="settings-worktree-actions">
              ...
              ` : `
                <button type="button" data-action="resume-worktree" data-repo-id="${escapeAttribute(repo.id)}" data-worktree-path="${escapeAttribute(entry.path)}">Resume</button>
              `}
              <button
                type="button"
                ...
                data-worktree-title="${escapeAttribute(entry.title)}"
              >Rename</button>
              <button
                type="button"
                ...
                data-repo-id="${escapeAttribute(repo.id)}"
                data-worktree-path="${escapeAttribute(entry.path)}"
                data-session-id="${escapeAttribute(entry.sessionId)}"
                data-worktree-kind="${escapeAttribute(entry.kind)}"
              >Delete</button>
              <button
                type="button"
        ...
        `).join("")
            : `<div class="empty-state">No ${ui.worktreeManagerFilter === "active" ? "active" : "non-active"} worktrees match this filter.</div>`
        }
      </div>
    </section>
  `;
...
async function handleClick(event) {
  ...
  switch (action) {
    ...
    case "worktree-filter":
      ui.worktreeManagerFilter = normalizeWorktreeManagerFilter(target.dataset.filterValue);
      renderDetail();
      if (settingsDialog.open) {
        await renderSettingsDialog();
      }
    ...
    case "delete-worktree":
      await deleteWorktreeFromManager(target);
      break;
    case "resume-worktree":
      await resumeWorktreeFromManager(target);
      break;
    case "rename-worktree":
      await renameWorktreeFromManager(target);
      break;
...
async function ensureRepoBranches(repoId: string | null | undefined): Promise<string[]> {
  if (!repoId) {
    return [];
  }

  const cached = ui.repoBranchCache.get(repoId);
  if (cached) {
    return cached.branches;
  }

  if (ui.repoBranchLoading.has(repoId)) {
    return [];
  }

  ui.repoBranchLoading.add(repoId);
  try {
    const branches = await api.listRepoBranches(repoId);
    const normalizedBranches = branches
      .map((branch) => branch.trim())
      .filter((branch) => !!branch)
      .sort((left, right) => left.localeCompare(right));
    ui.repoBranchCache.set(repoId, {
    ...
    ui.repoBranchLoading.delete(repoId);
  }
}

function repoBranchOptions(repo: RepoSnapshot, extraBranches: Array<string | null | undefined> = []): string[] {
  const branches = new Set<string>();
  for (const branch of ui.repoBranchCache.get(repo.id)?.branches || []) {
    if (branch) {
      branches.add(branch);
    }
  }
...
function renderBranchSelectOptions(
  ...
  const branches = repoBranchOptions(repo, [selected, ...extraBranches]);
  if (!branches.length) {
    return `<option value="${escapeAttribute(selected || "main")}">${escapeHtml(selected || "main")}</option>`;
  }

  return branches
    .map((branch) => `
      <option value="${escapeAttribute(branch)}" ${branch === selected ? "selected" : ""}>${escapeHtml(branch)}</option>
    `)
    .join("");
}
...
function normalizeWorktreeManagerFilter(value: string | null | undefined): WorktreeManagerFilter {
  return value === "active" || value === "inactive" ? value : "all";
}

function sessionNeedsWorktreeClosePrompt(session: SessionSummary | null | undefined): boolean {
  return session?.parallelWorktree?.mode === "isolated" && !!session.parallelWorktree.worktreePath;
}

async function promptWorktreeCloseAction(session: SessionSummary): Promise<WorktreeCloseAction | null> {
  const repo = repoById(session.repoID);
  ...
  ui.repoBranchCache.delete(repo.id);
  await ensureRepoBranches(repo.id);
  const selectedBranch =
    session.parallelWorktree.landingBranch ||
    effectiveRepoParallelWorktreeSettings(repo).landingBranch ||
    "main";

  worktreeCloseDialog.innerHTML = `
    <form method="dialog" class="dialog-body">
      <div class="dialog-header">
        <div>
          <div class="eyebrow">Worktree</div>
          <h2 class="dialog-title">Delete worktree?</h2>
          <div class="muted">${escapeHtml(session.title)} · ${escapeHtml(abbreviateHome(worktreePath))}</div>
        </div>
      </div>
      <label class="settings-inline-field">
        <span class="row-title">Push to Branch</span>
        <select id="worktree-close-branch">
          ${renderBranchSelectOptions(repo, selectedBranch, [session.parallelWorktree.branch])}
        </select>
      </label>
      <div class="dialog-actions">
        <button value="keep">No</button>
        <button value="delete">Yes, Delete</button>
        <button class="primary" value="push">Push to Branch</button>
      </div>
    </form>
  `;
  ...
  return new Promise((resolve) => {
    const handleClose = () => {
      const returnValue = worktreeCloseDialog.returnValue;
      const branchSelect = worktreeCloseDialog.querySelector<HTMLSelectElement>("#worktree-close-branch");
      worktreeCloseDialog.removeEventListener("close", handleClose);
      ...
      if (returnValue === "push") {
        resolve({
          kind: "push",
          branch: branchSelect?.value || selectedBranch
        });
        return;
      }
    ...
    worktreeCloseDialog.returnValue = "";
    worktreeCloseDialog.addEventListener("close", handleClose);
    worktreeCloseDialog.showModal();
  });
}

async function closeSessionById(sessionId) {
  const session = sessionById(sessionId);
  const worktreeAction = sessionNeedsWorktreeClosePrompt(session)
    ? await promptWorktreeCloseAction(session as SessionSummary)
    : undefined;
  if (worktreeAction === null) {
    return;
  }
  ...
  try {
    await api.closeSession(sessionId, worktreeAction);
  } catch (error) {
    window.alert(error instanceof Error ? error.message : "Failed to close session.");
    return;
  }
...
async function deleteWorktreeFromManager(target: HTMLElement) {
  ...
  const isActive = target.dataset.worktreeKind === "active";
  const message = isActive
    ? "Delete this worktree and close its active session?"
    : "Delete this retained worktree?";
  ...
  try {
    await api.deleteWorktree({
      repoId,
      worktreePath,
      sessionId: target.dataset.sessionId || undefined
    });
  } catch (error) {
    window.alert(error instanceof Error ? error.message : "Failed to delete worktree.");
  }
}

async function resumeWorktreeFromManager(target: HTMLElement) {
  const repoId = target.dataset.repoId;
  const worktreePath = target.dataset.worktreePath;
  if (!repoId || !worktreePath) {
    return;
  ...
  try {
    const sessionId = await api.resumeWorktree({ repoId, worktreePath });
    if (sessionId) {
      if (settingsDialog.open) {
    ...
    window.alert(error instanceof Error ? error.message : "Failed to resume worktree.");
  }
}

async function renameWorktreeFromManager(target: HTMLElement) {
  const repoId = target.dataset.repoId;
  const worktreePath = target.dataset.worktreePath;
  if (!repoId || !worktreePath) {
    return;
  ...
  const currentTitle = target.dataset.worktreeTitle || "";
  const nextTitle = window.prompt("Rename worktree", currentTitle);
  if (nextTitle === null) {
    return;
  }
  ...
  try {
    await api.renameWorktree({
      repoId,
      worktreePath,
      title: normalizedTitle
    });
  } catch (error) {
    window.alert(error instanceof Error ? error.message : "Failed to rename worktree.");
  }
}
...
async function openSettings(
  ...
  const repoId = currentRepoId();
  if (repoId) {
    ui.repoBranchCache.delete(repoId);
  }
  ...
  return dom(
    "button",
    {
      ...
      attrs: {
        ...
        "data-tooltip": voiceTooltip(voiceState),
        title: "Voice Mode",
...
function voiceCallIsActive() {
  return ui.voiceCallState === "connecting" || ui.voiceCallState === "listening";
}
...
async function startVoiceSession() {
  ...
  try {
    ...
    if (!pyCheck.found) {
      markVoiceError(
        "Python 3.10+ is required for voice mode. Install it from python.org or run: brew install python3"
      );
      return;
    }

    const deps = await api.installVoiceDeps();
    if (!voiceSessionIsCurrent(runId)) return;

    if (!deps.success) {
      console.warn("[Voice] dependency setup failed:", deps.error || "unknown error");
      markVoiceError(
        "Voice mode could not finish setup. Make sure Python 3.10+ can install packages with pip or uv, then retry."
      );
      return;
    }

    const result = await api.startVoiceCall(config);
    if (!voiceSessionIsCurrent(runId)) {
      if (result.success) {
        void api.stopVoiceCall();
      }
      return;
    ...
    if (!result.success || !result.port) {
      console.warn("[Voice] bot start failed:", result.error || "unknown error");
      markVoiceError(
        "Voice mode could not start. Check that the selected provider API keys are configured, then retry."
      );
      return;
    }

    await connectPipecatClient(result.port, runId);
  } catch (err) {
    if (!voiceSessionIsCurrent(runId)) return;
    const msg = err instanceof Error ? err.message : String(err);
    console.warn("[Voice] setup failed:", msg);
    markVoiceError("Voice mode could not start. Check your provider settings and retry.");
  }
}
    ...
    pipecatClient = new HydraSmallWebRTCClient({
      ...
      callbacks: {
        ...
        onLocalAudioLevel: (level: number) => {
          ...
          const pctEl = document.getElementById("voice-mic-pct");
          if (pctEl) {
...
async function saveVoiceConfig() {
  ...
  await api.updateVoiceConfig(nextConfig);
  state.preferences.voiceConfig = nextConfig;

  appendVoiceTranscript("system", "Settings saved. Voice mode will reconnect automatically.");
  renderVoiceDialog();
...
function renderVoiceDialog() {
  ...
  const canMute = voiceState === "listening" && !!pipecatClient;
  const hasTranscript = ui.voiceTranscript.length > 0;
  const isVoiceError = (e: { role: string; text: string }) =>
    e.role === "system" && (e.text.startsWith("Error:") || e.text.includes("could not") || e.text.includes("stopped unexpectedly") || e.text.includes("missing required"));
  const lastError = [...ui.voiceTranscript].reverse().find(isVoiceError);
  const hasInstallLog = ui.voiceInstallLog.length > 0;
  ...
  if (voiceState === "connecting") {
    ...
    statusTitle = "Setting up voice mode\u2026";
    statusDetail = "Checking Python, installing dependencies, and starting the voice agent.";
  } else if (voiceState === "error") {
    statusDot = "error";
    statusTitle = lastError ? "Setup failed" : "Voice error";
    statusDetail = "Fix the issue, then retry or change settings.";
  } else if (voiceState === "listening") {
    statusDot = "listening";
    statusTitle = "Listening";
    statusDetail = "Speak naturally. The voice agent is connected and ready.";
  }

  // Build the relevant API key fields based on selected providers
  const keyEntries: { key: string; label: string }[] = [];
  if (cfg.llmProvider === "openai") keyEntries.push({ key: "OPENAI_API_KEY", label: "OpenAI API key" });
  if (cfg.llmProvider === "anthropic") keyEntries.push({ key: "ANTHROPIC_API_KEY", label: "Anthropic API key" });
  if (cfg.llmProvider === "google" || cfg.sttProvider === "google") keyEntries.push({ key: "GOOGLE_API_KEY", label: "Google API key" });
  if (cfg.sttProvider === "deepgram" || cfg.ttsProvider === "deepgram") keyEntries.push({ key: "DEEPGRAM_API_KEY", label: "Deepgram API key" });
  if (cfg.ttsProvider === "cartesia") keyEntries.push({ key: "CARTESIA_API_KEY", label: "Cartesia API key" });
  if (cfg.ttsProvider === "elevenlabs") keyEntries.push({ key: "ELEVENLABS_API_KEY", label: "ElevenLabs API key" });
  // Deduplicate
  const seenKeys = new Set<string>();
  const uniqueKeyEntries = keyEntries.filter((e) => {
    if (seenKeys.has(e.key)) return false;
    seenKeys.add(e.key);
    return true;
  });

  const sel = (name: string, value: string) => value === (cfg as unknown as Record<string, unknown>)[name] ? "selected" : "";
  ...
  replaceDomChildren(
    ...
    dom(
      ...
      dom(
        ...
        dom(
          ...
          dom(
            ...
            dom("div", { className: "voice-header-label" }, "Voice Mode"),
            dom("h2", { className: "voice-header-title" }, "Hands-free Hydra control"),
            dom("div", { className: "voice-header-subtitle" },
        ...
        dom(
          ...
          statusDot
            ? dom(
                ...
                dom("div", { className: classNames("voice-status-dot", statusDot) }),
                dom(
                  "div",
                  { className: "voice-status-info" },
                  dom("div", { className: "voice-status-title" }, statusTitle),
                  dom("div", { className: "voice-status-detail" }, statusDetail)
                )
              )
            : null,
          // Error detail bar
          voiceState === "error" && lastError
            ? dom("div", { className: "voice-error-bar" }, lastError.text)
            : null,
          // Transcript
          dom(
            "div",
            ...
            hasTranscript
              ? null
              : dom("div", { className: "voice-transcript-empty" }, "Voice activity and transcripts will appear here."),
            ...ui.voiceTranscript
              .filter((e) => !isVoiceError(e))
              .map((entry) =>
                dom(
                  "div",
          ...
          hasInstallLog
            ? trustedFragment(`
                <details class="voice-install-details">
                  <summary>Dependency install log</summary>
                  <div class="voice-install-log-content">${ui.voiceInstallLog.map((l) => escapeAttribute(l)).join("\n")}</div>
                </details>
              `)
            : null
        ),
        ...
        dom(
          ...
          dom("span", { className: "voice-mic-label" }, "Mic level"),
          dom(
            "div",
            { className: "voice-mic-track" },
            dom("div", { className: "voice-mic-track-fill", attrs: { id: "voice-mic-bar" } })
          ),
          dom("span", { className: "voice-mic-pct", attrs: { id: "voice-mic-pct" } }, "0%"),
          dom(
            "div",
            { className: "voice-controls-right" },
            canMute
              ? dom("button", {
                  className: classNames("voice-btn", ui.voiceMuted ? "voice-btn-muted" : undefined),
                  attrs: { "data-action": "voice-mute-toggle" }
                }, ui.voiceMuted ? "Unmute" : "Mute")
              : null,
            voiceState === "error"
      ...
      dom(
        ...
        trustedFragment(`
          <div class="voice-settings-scroll">
            <form id="voice-settings-form">
              <div class="voice-settings-label">Settings</div>
              <div class="voice-settings-heading">Voice stack</div>

              <div class="voice-field">
                <label class="voice-field-label">LLM</label>
                <select name="llmProvider">
                  <option value="openai" ${sel("llmProvider", "openai")}>OpenAI</option>
                  <option value="anthropic" ${sel("llmProvider", "anthropic")}>Anthropic</option>
                  <option value="google" ${sel("llmProvider", "google")}>Google</option>
                  <option value="ollama" ${sel("llmProvider", "ollama")}>Ollama (local)</option>
                </select>
              </div>

              <div class="voice-field">
                <label class="voice-field-label">STT</label>
                <select name="sttProvider">
                  <option value="deepgram" ${sel("sttProvider", "deepgram")}>Deepgram</option>
                  <option value="google" ${sel("sttProvider", "google")}>Google</option>
                  <option value="whisper" ${sel("sttProvider", "whisper")}>Whisper (local)</option>
                </select>
              </div>

              <div class="voice-field">
                <label class="voice-field-label">TTS</label>
                <select name="ttsProvider">
                  <option value="cartesia" ${sel("ttsProvider", "cartesia")}>Cartesia</option>
                  <option value="elevenlabs" ${sel("ttsProvider", "elevenlabs")}>ElevenLabs</option>
                  <option value="deepgram" ${sel("ttsProvider", "deepgram")}>Deepgram</option>
                </select>
              </div>

              <div class="voice-field">
                <label class="voice-field-label">TTS voice</label>
                <input type="text" name="ttsVoice" value="${escapeAttribute(cfg.ttsVoice || "")}" placeholder="Provider default" />
              </div>

              <div class="voice-toggle-row">
                <input type="checkbox" id="voice-subagents-cb" name="enableSubagents" ${cfg.enableSubagents ? "checked" : ""} />
                <label for="voice-subagents-cb">Enable subagents</label>
              </div>

              <div class="voice-settings-divider"></div>
              <div class="voice-settings-section-title">Provider keys</div>

              ${uniqueKeyEntries.map((entry) => `
                <div class="voice-key-field">
                  <label class="voice-key-label">${entry.label}</label>
                  <input type="password" name="${entry.key}" value="${escapeAttribute((cfg.apiKeys as Record<string, string>)[entry.key] || "")}" />
                </div>
              `).join("")}

              <button type="button" class="voice-save-btn" data-action="voice-save-config">
                Save &amp; Reconnect
              </button>
            </form>
          </div>
